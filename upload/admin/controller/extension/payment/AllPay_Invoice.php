...
// 執行發票作業項目。
abstract class OPayInvoiceMethod
{
	// 一般開立發票。
...
// 延遲註記
abstract class OPayDelayFlagType
{
	// 延遲註記
	const Delay = '1';
...
// 通知類別 
abstract class OPayAllowanceNotifyType
{
	// 簡訊通知
	const Sms = 'S';
...
// 發送內容類型
abstract class OPayInvoiceTagType
{
	// 發票開立
	const Invoice = 'I';
...
// 發送對象
abstract class OPayNotifiedType
{
	// 通知客戶
	const Customer = 'C';
...
{	
	...
	function __construct(){
			...
			'CustomerEmail' => '',
			'ClearanceMark' => '',
			'Print' => OPayPrintMark::No,
			'Donation' => OPayDonation::No,
			'LoveCode' => '',
			'CarruerType' => OPayCarruerType::None,
			'CarruerNum' => '',
			'TaxType' => '',
			'SalesAmount' => '',
			'InvoiceRemark' => '',
		...
		return Opay_Invoice_Send::CheckOut($arParameters, $this->HashKey, $this->HashIV, $this->Invoice_Method, $this->Invoice_Url);
    	}	
}
...
{ 	
	...
	*/
	static function CheckOut($arParameters = array(), $HashKey='', $HashIV='', $Invoice_Method = '', $ServiceURL=''){
		...
		$szResult = OPay_IO::ServerPost($arParameters, $ServiceURL);
	...
	protected static function process_send($arParameters = array(), $HashKey = '', $HashIV = '', $Invoice_Method = '', $ServiceURL = ''){
		...
		//宣告物件
		$InvoiceMethod    = 'Opay_'.$Invoice_Method;
		...
		// 2檢查共用參數
		Opay_Invoice_Send::check_string($arParameters['MerchantID'], $HashKey, $HashIV, $Invoice_Method, $ServiceURL);

		// 3檢查各別參數
		$arParameters = self::$InvoiceObj->check_extend_string($arParameters);

		// 4處理需要轉換為urlencode的參數
		$arParameters = Opay_Invoice_Send::urlencode_process($arParameters, self::$InvoiceObj->urlencode_field);
		
		// 5欄位例外處理方式(送壓碼前)
		$arException = self::$InvoiceObj->check_exception($arParameters);

		// 6產生壓碼
		$arParameters['CheckMacValue'] = Opay_Invoice_Send::generate_checkmacvalue($arException, self::$InvoiceObj->none_verification, $HashKey, $HashIV);
	...
	*/
	protected static function process_return($sParameters = '', $HashKey = '', $HashIV = '', $Invoice_Method = ''){
		...
		// 7字串轉陣列
		$arParameters = Opay_Invoice_Send::string_to_array($sParameters);
		...
		if(isset($arParameters['CheckMacValue'])){
			$CheckMacValue = Opay_Invoice_Send::generate_checkmacvalue($arException, self::$InvoiceObj_Return->none_verification, $HashKey, $HashIV);
		...
		// 10處理需要urldecode的參數
		$arParameters = Opay_Invoice_Send::urldecode_process($arParameters, self::$InvoiceObj_Return->urlencode_field);
    	...
    	protected static function urlencode_process($arParameters = array(), $urlencode_field = array()){
    		...
    		foreach($arParameters as $key => $value)
    		{
    			if(isset($urlencode_field[$key]))
    			{
    				...
    				$arParameters[$key] = OPay_CheckMacValue::Replace_Symbol($arParameters[$key]);
    			}
    		}
	...
	*/
    	protected static function generate_checkmacvalue($arParameters = array(), $none_verification = array(), $HashKey = '', $HashIV = ''){
    		...
    		$sCheck_MacValue = OPay_CheckMacValue::generate($arParameters, $HashKey, $HashIV, OPayEncryptType::ENC_MD5);
	...
	*/
    	protected static function urldecode_process($arParameters = array(), $urlencode_field = array()){
    		...
    		foreach($arParameters as $key => $value)
    		{
    			if(isset($urlencode_field[$key]))
    			{
    				$arParameters[$key] = OPay_CheckMacValue::Replace_Symbol_Decode($arParameters[$key]);
...
class Opay_INVOICE
{
	...
	*/
		...
		if(strlen($arParameters['CustomerID']) > 20 )
        	...
        	if ($arParameters['Print'] == OPayPrintMark::Yes)
		{
		...
		if ($arParameters['TaxType'] == OPayTaxType::Zero)
		{
			if ( ( $arParameters['ClearanceMark'] != OPayClearanceMark::Yes ) && ( $arParameters['ClearanceMark'] != OPayClearanceMark::No ) )
			{
		...
		else
		{
        	...
        	// *列印註記僅能為 0 或 1
		if ( ( $arParameters['Print'] != OPayPrintMark::Yes ) && ( $arParameters['Print'] != OPayPrintMark::No ) )
		{
			array_push($arErrors, "12:Invalid Print.");
		}
		// *若捐贈註記 = '1' (捐贈)時，則VAL = '0' (不列印)
		if ($arParameters['Donation'] == OPayDonation::Yes)
		{
			if ($arParameters['Print'] != OPayPrintMark::No)
			{
		...
		// 線下列印判斷
		// 當線下廠商開立發票無載具且無統一編號時，必須列印。
		if($arParameters['OnLine'] === false)
		{
			if( $arParameters['CarruerType'] == OPayCarruerType::None && strlen($arParameters['CustomerIdentifier']) == 0 )
			{
				if ($arParameters['Print'] != OPayPrintMark::Yes)
				{
		...
		if ( ( $arParameters['CarruerType'] != OPayCarruerType::None ) && ( $arParameters['CarruerType'] != OPayCarruerType::Member ) && ( $arParameters['CarruerType'] != OPayCarruerType::Citizen ) && ( $arParameters['CarruerType'] != OPayCarruerType::Cellphone ) )
		{
		...
		else
		{
			...
			if (strlen($arParameters['CustomerIdentifier']) > 0)
			{
				if ($arParameters['CarruerType'] == OPayCarruerType::Member || $arParameters['CarruerType'] == OPayCarruerType::Citizen )
				{
                  			array_push($arErrors, "15:Invalid CarruerType.");
              			}		
			}
		...
		if ( ( $arParameters['TaxType'] != OPayTaxType::Dutiable ) && ( $arParameters['TaxType'] != OPayTaxType::Zero ) && ( $arParameters['TaxType'] != OPayTaxType::Free ) && ( $arParameters['TaxType'] != OPayTaxType::Mix ) )
		{
		...
		if( ( $arParameters['InvType'] != OPayInvType::General ) && ( $arParameters['InvType'] != OPayInvType::Special ) )
		{
		...
		{
			if( ( $arParameters['vat'] != OPayVatType::Yes ) && ( $arParameters['vat'] != OPayVatType::No ) )
			{
...
class Opay_INVOICE_DELAY
{
	...
	*/
		...
		// 30.延遲註記 DelayFlag
		if( ( $arParameters['DelayFlag'] != OPayDelayFlagType::Delay ) && ( $arParameters['DelayFlag'] != OPayDelayFlagType::Trigger ) )
		{
		...
		if ( $arParameters['DelayFlag'] == OPayDelayFlagType::Delay )
		{
		...
		if ($arParameters['DelayFlag'] == OPayDelayFlagType::Trigger)
		{
		...
		if( $arParameters['PayType'] != OPayPayTypeCategory::Allpay )
		{
        	...
        	{
        		// *必填項目 交易類別名稱預設不能為空值 僅允許 ALLPAY
        		$arParameters['PayAct'] = 'ALLPAY' ;
        	}
...
class Opay_ALLOWANCE
{
	...
	*/
		...
		// *固定給定下述預設值
		if( ( $arParameters['AllowanceNotify'] != OPayAllowanceNotifyType::Sms ) && ( $arParameters['AllowanceNotify'] != OPayAllowanceNotifyType::Email ) && ( $arParameters['AllowanceNotify'] != OPayAllowanceNotifyType::All ) && ( $arParameters['AllowanceNotify'] != OPayAllowanceNotifyType::None ) )
		{
		...
		if($arParameters['AllowanceNotify'] == OPayAllowanceNotifyType::Email && strlen($arParameters['NotifyMail']) == 0 )
		{
		...
		if( $arParameters['AllowanceNotify'] == OPayAllowanceNotifyType::Sms && strlen($arParameters['NotifyPhone']) == 0 )
		{
			array_push($arErrors, "40:NotifyPhone is required.");
		}

		// 39-40 通知電子信箱、通知手機號碼不能全為空值 (如果狀態為SMS 或 EMAIL)
		if(strlen($arParameters['NotifyPhone']) == 0 && strlen($arParameters['NotifyMail']) == 0 && ( $arParameters['AllowanceNotify'] == OPayAllowanceNotifyType::Sms || $arParameters['AllowanceNotify'] == OPayAllowanceNotifyType::Email ) )
		{
		...
		else
		{
			...
			if( $arParameters['AllowanceNotify'] == OPayAllowanceNotifyType::All && ( strlen($arParameters['NotifyMail']) == 0 || strlen($arParameters['NotifyPhone']) == 0 ) )
			{
				array_push($arErrors, "39-40:NotifyMail And NotifyPhone is required.");
			}
			
			// *下述情況通知手機號碼與電子信箱為空值(通知類別為N-皆不通知)
			if($arParameters['AllowanceNotify'] == OPayAllowanceNotifyType::None && ( strlen($arParameters['NotifyMail']) > 0 || strlen($arParameters['NotifyPhone']) > 0 ))
			{
...
class Opay_ALLOWANCE_VOID
{
	...
	*/
		...
		{
			//array_push($arErrors, '44:AllowanceNo length as 16. AllowanceNo =>' . $arParameters['AllowanceNo']);
		}
...
*  G查詢作廢發票
*/
class Opay_INVOICE_VOID_SEARCH
{
    	// 所需參數
    	public $parameters = array(
            	'TimeStamp'		=>'',
...
*  I查詢折讓作廢明細
*/
class Opay_ALLOWANCE_VOID_SEARCH
{
    	// 所需參數
    	public $parameters = array(
            	'TimeStamp'		=>'',
...
class Opay_INVOICE_NOTIFY
{
	...
	*/
		...
		// 44.折讓編號 AllowanceNo
		if( ( $arParameters['InvoiceTag'] == OPayInvoiceTagType::Allowance ) || ( $arParameters['InvoiceTag'] == OPayInvoiceTagType::Allowance_Void ) )
		{	
			if(strlen($arParameters['AllowanceNo']) == 0)
			{
				array_push($arErrors, "44:AllowanceNo is required.");
		...
		if( $arParameters['Notify'] == OPayNotifyType::Email && strlen($arParameters['NotifyMail']) == 0 )
		{
			array_push($arErrors, "39:NotifyMail is required.");
		}
		...
		if( $arParameters['Notify'] == OPayNotifyType::Sms && strlen($arParameters['Phone']) == 0 )
		{
		...
		{
			if( $arParameters['Notify'] == OPayNotifyType::All && ( strlen($arParameters['NotifyMail']) == 0 || strlen($arParameters['Phone']) == 0 ) )
			{
		...
		// *固定給定下述預設值
		if( ($arParameters['Notify'] != OPayNotifyType::Sms ) && ( $arParameters['Notify'] != OPayNotifyType::Email ) && ( $arParameters['Notify'] != OPayNotifyType::All ) )
		{
		...
		// *固定給定下述預設值
		if( ( $arParameters['InvoiceTag'] != OPayInvoiceTagType::Invoice ) && ( $arParameters['InvoiceTag'] != OPayInvoiceTagType::Invoice_Void ) && ( $arParameters['InvoiceTag'] != OPayInvoiceTagType::Allowance ) && ( $arParameters['InvoiceTag'] != OPayInvoiceTagType::Allowance_Void ) && ( $arParameters['InvoiceTag'] != OPayInvoiceTagType::Invoice_Winning ) )
		{
		...
		// 49.發送對象 Notified
		// *固定給定下述預設值
		if( ( $arParameters['Notified'] != OPayNotifiedType::Customer ) && ( $arParameters['Notified'] != OPayNotifiedType::vendor ) && ( $arParameters['Notified'] != OPayNotifiedType::All ) )
		{
...
*  L手機條碼驗證
*/
class Opay_CHECK_MOBILE_BARCODE
{
    	// 所需參數
    	public $parameters = array(
            	'TimeStamp'		=>'',
...
if(!class_exists('OPay_CheckMacValue'))
{
	/**
	*  檢查碼
	*/
	class OPay_CheckMacValue
	{
		/**
		* 產生檢查碼
		*/
		static function generate($arParameters = array(), $HashKey = '', $HashIV = '', $encType = 0){
			...
			if(isset($arParameters)){
				...
				uksort($arParameters, array('OPay_CheckMacValue','merchantSort'));
				...
				// 取代為與 dotNet 相符的字元
				$sMacValue = OPay_CheckMacValue::Replace_Symbol($sMacValue);
